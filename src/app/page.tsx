...
interface SectionConfig {
  id: string;
  type: 'markdown' | 'publications' | 'list' | 'questions';
  title?: string;
  source?: string;
  filter?: string;
  limit?: number;
...
function processSections(sections: SectionConfig[], locale?: string): SectionConfig[] {
  return sections.map((section: SectionConfig) => {
    switch (section.type) {
      ...
      case 'publications': {
        const bibtex = getBibtexContent(section.source || 'publications.bib', locale);
      ...
      case 'list': {
        ...
        return {
          ...section,
          items: section.limit ? newsItems.slice(0, section.limit) : newsItems,
        };
      }
      case 'questions': {
        const questionsPage = section.source ? getTomlContent<QuestionsPageConfig>(section.source, locale) : null;
        const questions = sortQuestions(questionsPage?.items || []);
        return {
          ...section,
          questions: section.limit ? questions.slice(0, section.limit) : questions,
        };
      }
      default:
        return section;
  ...
  if (enableOnePageMode) {
    pagesToShow = localeConfig.navigation
      ...
      .map((item) => {
        ...
        if (pageConfig.type === 'questions') {
          const questionsConfig = pageConfig as QuestionsPageConfig;
          return {
            type: 'questions',
            id: item.target,
            config: {
              ...questionsConfig,
              items: sortQuestions(questionsConfig.items || []),
            },
          } as PageData;
        }
