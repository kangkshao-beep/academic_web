...
const InspireIcon = ({ className }: { className?: string }) => (
    <svg
        viewBox="0 0 24 24"
        fill="none"
        className={className}
...
export default function Profile({ author, social, features, researchInterests }: ProfileProps) {
    ...
    const likeStorageKey = 'academic-site-user-liked';
    const advisors = author.advisors ?? [];
    ...
    useEffect(() => {
        ...
        const userHasLiked = localStorage.getItem(likeStorageKey);
        if (userHasLiked === 'true') {
            setHasLiked(true);
        }
    }, [features.enable_likes, likeStorageKey]);
    ...
    return (
        <motion.div
            ...
            className="sticky top-8 scroll-mt-24"
        >
            {/* Profile Image */}
            ...
            <div className="text-center mb-6">
                ...
                {advisors.length > 0 && (
                    <p className="text-sm text-neutral-600 dark:text-neutral-400">
                        <span className="font-medium text-primary">{messages.profile.advisors}:</span>{' '}
