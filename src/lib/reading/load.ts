...
export type ReadingLoadFailure = 'authentication' | 'malformed' | 'unavailable';
...
async function fetchJson(filename: string, signal: AbortSignal): Promise<unknown> {
  ...
  try {
    ...
    throw new ReadingLoadError('unavailable', '无法连接到私有 Reading 数据服务。');
  }

  if (response.status === 401 || response.status === 403) {
    throw new ReadingLoadError('authentication', 'Reading 登录已失效或当前凭据无权读取数据。');
  }
  if (!response.ok) {
