...
const THREADS_KEYS = ['schema_version', 'visibility', 'generated_on', 'threads'] as const;
const THREAD_KEYS = [
  'id',
  'title',
  'visibility',
...
function uri(value: unknown, path: string): string {
  ...
  try {
    ...
    if (parsed.protocol !== 'https:' && parsed.protocol !== 'http:') invalid(path);
  } catch {
    invalid(path);
  }
  return result;
...
function parseLibrary(value: unknown): ReadingLibrary {
  const item = object(value, 'library');
  exactKeys(item, LIBRARY_KEYS, 'library');
  ...
  const papers = array(item.papers, 'library.papers').map(parsePaper);
  if (papers.length === 0) invalid('library.papers');

  return {
    schema_version: oneOf(item.schema_version, [READING_SCHEMA_VERSION], 'library.schema_version'),
...
function parseEvidence(value: unknown, path: string): RelationEvidence {
  const item = object(value, path);
  ...
  exactKeys(item, THESIS_EVIDENCE_KEYS, path);
  return {
    kind,
    document_id: string(item.document_id, `${path}.document_id`),
    chapter: integer(item.chapter, `${path}.chapter`, 1, 7),
    section: string(item.section, `${path}.section`),
    printed_pages: integerList(item.printed_pages, `${path}.printed_pages`, {
      itemMin: 1,
...
function parseRelation(value: unknown, index: number): ReadingRelation {
  const path = `relations.edges[${index}]`;
  const item = object(value, path);
  exactKeys(item, EDGE_KEYS, path);
  const evidence = array(item.evidence, `${path}.evidence`).map((entry, evidenceIndex) =>
    parseEvidence(entry, `${path}.evidence[${evidenceIndex}]`)
  );
...
function parseRelations(value: unknown): ReadingRelations {
  const item = object(value, 'relations');
  exactKeys(item, RELATIONS_KEYS, 'relations');
  ...
  return {
    schema_version: oneOf(item.schema_version, [READING_SCHEMA_VERSION], 'relations.schema_version'),
    ...
    edges: array(item.edges, 'relations.edges').map(parseRelation),
    hypothesis_edges: [],
  };
}

function parseThread(value: unknown, index: number): ReadingThread {
  const path = `threads.threads[${index}]`;
  const item = object(value, path);
  exactKeys(item, THREAD_KEYS, path);
...
function parseThreads(value: unknown): ReadingThreads {
  const item = object(value, 'threads');
  exactKeys(item, THREADS_KEYS, 'threads');
  const threads = array(item.threads, 'threads.threads').map(parseThread);
  if (threads.length === 0) invalid('threads.threads');
  return {
    schema_version: oneOf(item.schema_version, [READING_SCHEMA_VERSION], 'threads.schema_version'),
...
function parseViewConfig(value: unknown): ReadingViewConfig {
  const item = object(value, 'view_config');
  const graph = object(item.graph, 'view_config.graph');
  exactKeys(item, VIEW_CONFIG_KEYS, 'view_config');
  exactKeys(graph, VIEW_GRAPH_KEYS, 'view_config.graph');
  ...
  const config: ReadingViewConfig = {
    schema_version: oneOf(item.schema_version, [READING_SCHEMA_VERSION], 'view_config.schema_version'),
    visibility: oneOf(item.visibility, ['private'], 'view_config.visibility'),
    default_view: oneOf(item.default_view, ['library', 'map', 'threads'], 'view_config.default_view'),
    graph: {
...
function validateInvariants(bundle: ReadingBundle): void {
  ...
  const sourceDocumentId = bundle.library.source_document.id;
  ...
  for (const [edgeIndex, edge] of bundle.relations.edges.entries()) {
    ...
    for (const evidence of edge.evidence) {
      ...
      if (
        evidence.document_id !== sourceDocumentId ||
  ...
  if (
    bundle.viewConfig.graph.initial_focus_ids.length > bundle.viewConfig.graph.initial_node_limit ||
  ...
  if (
    ...
    bundle.viewConfig.graph.hypothesis_layer_default ||
...
export function validateReadingBundle(input: {
  ...
  const bundle: ReadingBundle = {
    library: parseLibrary(input.library),
    relations: parseRelations(input.relations),
    threads: parseThreads(input.threads),
    viewConfig: parseViewConfig(input.viewConfig),
  };
