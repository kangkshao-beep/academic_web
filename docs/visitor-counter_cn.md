...
3. 在当前 Pages 项目的 **Settings -> Bindings** 中添加 D1 绑定：变量名必须是 `TRAFFIC_DB`，并选择刚建立的数据库。只需绑定 Production 环境。
4. 在同一 Pages 项目的 Production 环境变量中添加 `TRAFFIC_ENABLED=true`。
5. 重新部署 `main` 分支。部署完成后，访问网站即可开始计数。
